...
var CLEAR_SCREEN_HOME = "\x1B[2J\x1B[1;1H";
var CLEAR_LINE = "\x1B[2K";
var cursorAt = (row, col) => `\x1B[${row};${col}H`;
var setScrollingRegion = (top, bottom) => `\x1B[${top};${bottom}r`;
var RESET_SCROLLING_REGION = "\x1B[r";
var writeErr = (s) => {
...
var inlineSubs = (s) => {
  ...
  return s.replace(BOLD_PAT, (_m, inner) => `${BOLD}${inner}${RESET}`).replace(INLINE_CODE, (_m, inner) => `${DIM}${inner}${RESET}`);
};
var transformMarkdownLine = (line) => {
...
var piEventToText = (jsonLine) => {
  ...
  const t = typeof evt["type"] === "string" ? evt["type"] : null;
  if (t === "thinking_delta" || t === "text_delta") {
    const d = evt["delta"];
    return typeof d === "string" ? d : "";
  }
  if (t === "tool_use_start") {
    const direct = typeof evt["name"] === "string" ? evt["name"] : null;
    const tool = evt["tool"] ?? {};
    const nested = typeof tool["name"] === "string" ? tool["name"] : null;
  ...
  return new Promise((resolve) => {
    ...
    child.on("close", () => {
      if (combined.includes("Failed to load extension")) {
        const offenders = combined.split("\n").filter((l) => l.includes("Failed to load extension") || l.startsWith("Error")).join("\n");
...
var STORY_RE = /^story-\d+-\d+$/;
var load = (path) => {
  const text = readFileSync(path, "utf8");
  const data = (0, import_yaml.parse)(text) ?? {};
  return {
    development_status: data.development_status ?? {},
    dependencies: data.dependencies ?? {}
  };
};
var storyStatus = (s, id) => s.development_status[id] ?? "missing";
var nextStory = (s) => {
  ...
  const doneIds = new Set(
    Object.entries(ds).filter(([sid, st]) => STORY_RE.test(sid) && st === "done").map(([sid]) => sid)
  );
  for (const [sid, status] of Object.entries(ds)) {
    if (!STORY_RE.test(sid)) continue;
...
var forceStatus = (path, storyId, newStatus) => {
  const text = readFileSync(path, "utf8");
...
var countByStatus = (s) => {
  const stories = Object.entries(s.development_status).filter(
    ([sid]) => STORY_RE.test(sid)
  );
...
// src/flow-auto.ts
var HEADER_LINES = 12;
var STATUS_PATH = process.argv[2] ?? ".agents/implementation/sprint-status.yaml";
var PI_BIN = process.env["PI_BIN"] ?? "pi";
var PI_MODE = process.env["PI_MODE"] === "text" ? "text" : "json";
var PI_RAW = process.env["PI_RAW"] === "1";
var NO_STICKY = process.env["NO_STICKY_HEADER"] !== void 0;
process.env["FLOW_AUTO"] = "1";
if (process.env["FLOW_PARALLEL"] === void 0) {
  process.env["FLOW_PARALLEL"] = "1";
}
var FLOW_PARALLEL = process.env["FLOW_PARALLEL"];
var BATCH_PROMPT = `BATCH MODE ACTIVE. You have NO interactive stdin.
...
var progressBar = (done, total, width = 40) => {
...
var formatHeaderLines = (s) => {
  const { RESET, BOLD, DIM, GREEN, YELLOW, BLUE, GRAY, CYAN } = colors;
  const counts = countByStatus(s);
  const pct = counts.total > 0 ? Math.floor(counts.done * 100 / counts.total) : 0;
  const bar = progressBar(counts.done, counts.total);
  const epic = currentEpic(s);
  const epicLine = epic !== null ? `${BOLD}Current epic${RESET}  ${CYAN}${epic.id}${RESET}  ${DIM}(${epic.status} \u2014 ${epic.done}/${epic.total})${RESET}` : "";
  const lines = [
    `${BOLD}${CYAN}\u256D\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u256E${RESET}`,
...
`
  ...
  const doneIds = new Set(
    Object.entries(s.development_status).filter(([sid, st]) => /^story-\d+-\d+$/.test(sid) && st === "done").map(([sid]) => sid)
  );
  const STORY_RE2 = /^story-\d+-\d+$/;
  const isActionable = (sid, st) => {
  ...
  const actionable = Object.entries(s.development_status).filter(([sid, st]) => STORY_RE2.test(sid) && isActionable(sid, st)).slice(0, 5);
...
var runPhase = async (phase, story) => {
...
var ensureStatusTransition = (sid, expected) => {
  const s = load(STATUS_PATH);
...
var hasSeniorReviewSection = (storyPath) => {
  if (!existsSync(storyPath)) return false;
  const text = readFileSync2(storyPath, "utf8");
  return /^#{1,3}\s+Senior Review/m.test(text);
};
var printSummary = (storiesDone, elapsedSec) => {
...
`
    ...
    if (storyStatus(load(STATUS_PATH), sid) === "backlog") {
      await runPhase("flow-story", sid);
      ensureStatusTransition(sid, "ready-for-dev");
    }
    {
      const st = storyStatus(load(STATUS_PATH), sid);
      if (st === "ready-for-dev" || st === "in-progress") {
        await runPhase("flow-dev", sid);
        const stAfter = storyStatus(load(STATUS_PATH), sid);
    ...
    const phase3Status = storyStatus(load(STATUS_PATH), sid);
...
`);
    ...
    if (storyStatus(load(STATUS_PATH), sid) === "review") {
...
`);
    refreshHeader(load(STATUS_PATH));
  }
};
var code = await main().catch((e) => {
  stickyReset();
